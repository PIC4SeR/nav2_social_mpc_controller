...
#ifndef MPC_BASE__OVERALL_SOCIAL_COST_FUNCTION_HPP_
#define MPC_BASE__OVERALL_SOCIAL_COST_FUNCTION_HPP_
...
public:
  ...
  {
    T min_distance = T(1000.0);  // Initialize minimum distance to a large but reasonable value
    Eigen::Matrix<T, 2, 1> mePos(me[0], me[1]);        // Extract the position of the robot
    ...
    for (Eigen::Index i = 0; i < agents.cols(); i++)  // Iterate through each agent
    {
      ...
      // Clamp squared_distance to avoid numerical issues
      squared_distance = ceres::fmax(squared_distance, T(1e-6));
      
      min_distance = ceres::fmin(min_distance, squared_distance);  // Update the minimum distance
      found_valid_agent = true;
    }
    
    // If no valid agents found, return zero cost
