...
public:
  ...
  bool operator()(T const* const* parameters, T* residuals) const
  {
    ...
    Eigen::Matrix<T, 2, 1> robot_velocity(robot_block[0] * ceres::cos(robot_theta),
                                          robot_block[0] * ceres::sin(robot_theta));
    ...
    for (unsigned int agent_idx = 0; agent_idx < agent_count_; ++agent_idx)
    {
      ...
      Eigen::Matrix<T, 2, 1> current_velocity(current_agent_block[idx], current_agent_block[idx + 1]);
      Eigen::Matrix<T, 2, 1> previous_velocity(
          previous_agent_block != nullptr ? previous_agent_block[idx] : T(reference_velocities_[idx]),
          previous_agent_block != nullptr ? previous_agent_block[idx + 1] : T(reference_velocities_[idx + 1]));
      ...
      Eigen::Matrix<T, 2, 1> reference_velocity(T(reference_velocities_[idx]), T(reference_velocities_[idx + 1]));
      sfm_accel += (reference_velocity - current_velocity) / T(sfm_relaxation_time_);
      clampAcceleration(sfm_accel);

      residuals[idx] = T(sqrt_weight_) * (optimized_accel[0] - sfm_accel[0]);
      residuals[idx + 1] = T(sqrt_weight_) * (optimized_accel[1] - sfm_accel[1]);
    }
...
private:
  ...
  Eigen::Matrix<T, 2, 1> computeSocialAcceleration(
      ...
      const Eigen::Matrix<T, 6, Eigen::Dynamic>& agents, const T* const current_agent_block,
      unsigned int agent_idx) const
  {
    ...
    for (unsigned int other_idx = 0; other_idx < agent_count_; ++other_idx)
    {
      ...
      const unsigned int idx = 2 * other_idx;
      const Eigen::Index col = static_cast<Eigen::Index>(other_idx);
      Eigen::Matrix<T, 2, 1> other_position(agents(0, col), agents(1, col));
      Eigen::Matrix<T, 2, 1> other_velocity(current_agent_block[idx], current_agent_block[idx + 1]);
                                                    ...
                                                    const Eigen::Matrix<T, 2, 1>& other_velocity) const
  {
    ...
    T theta = ceres::atan2(ceres::sin(ceres::atan2(diff_direction[1], diff_direction[0]) -
                                      ceres::atan2(interaction_direction[1], interaction_direction[0])),
                           ceres::cos(ceres::atan2(diff_direction[1], diff_direction[0]) -
                                      ceres::atan2(interaction_direction[1], interaction_direction[0])));
    ...
    Eigen::Matrix<T, 2, 1> left_normal(-interaction_direction[1], interaction_direction[0]);
