...
public:
  DistanceCostFunction(double weight, const Eigen::Matrix<double, 2, 1> &goal)
      : weight_(weight), goal_(goal) {}
  ...
  bool operator()(const T *const state, T *residual) const {
    ...
    Eigen::Matrix<T, 2, 1> p_ori((T)goal_[0], (T)goal_[1]);
    residual[0] =
  ...
  double weight_;
  const Eigen::Matrix<double, 2, 1> goal_;
};
